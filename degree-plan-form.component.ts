...
export class DegreePlanFormComponent implements OnInit {
  ...
  onSubmit(degreeForm: NgForm){
	...
	this.courses$ = this.dataService.getSummerCourses();
	for(let sc of summerCourses)
		console.log(sc);
	...
	for(let quarter of degreePlan) {
		...
		if(!allFoundationCoursesAdded){
			...
			foundationCourseSet.forEach(function(fc) {
				if(/* DATABASE CALL: courseAvailThisTerm(fc, quarter.term) && */ cpqCounter < numCPQ && !courseSet.has(fc)){
                    courseSet.add(fc);
		...
		if(!allSpecificAreaCoursesAdded){
			...
			for(var i = 0; i < this.csSpecificAreaCourses[degreeRqmnts[0]].length && cpqCounter < numCPQ && saCounter < 4; i++) {
				if(/* DATABASE CALL: courseAvailThisTerm(sa, quarter.term) && */ !courseSet.has(this.csSpecificAreaCourses[degreeRqmnts[0]][i])) //changed from 1 to 0
                {
		...
		if(!allAnyAreaCoursesAdded){
			...
			for(var i = 0; i < this.csSpecificAreaCourses.length && cpqCounter < numCPQ && aaCounter < 4; i++) {
                ...
                for(var j = 0; j < courseArea.length && cpqCounter < numCPQ && aaCounter < 4; j++){
				    if(/* DATABASE CALL: courseAvailThisTerm(courseArea[j], quarter.term) && */ !courseSet.has(courseArea[j])){
  ...
  fillRqmntsIS(numCPQ : number, degreeRqmnts : number[], degreePlan : Array<Quarter>){
    ...
    for(let quarter of degreePlan) {
        ...
        if(!allAdvancedCoursesAdded){
            advancedCourseSet.forEach(function(fc) {
                if(/* DATABASE CALL: courseAvailThisTerm(fc, quarter.term) && */ cpqCounter < numCPQ && !courseSet.has(fc)){
                    courseSet.add(fc);
                    quarter.addCourse(fc);
                    foundationCourseSet.delete(fc);
        ...
        if(!allMajorElectiveCoursesAdded){
            for(var i = 0; i < this.isMajorElectiveCourses[degreeRqmnts[0]].length && cpqCounter < numCPQ && meCounter < degreeRqmnts[1]; i++) {
				    if(/* DATABASE CALL: courseAvailThisTerm(this.isMajorElectiveCourses[degreeRqmnts[0]][i], quarter.term) && */  !courseSet.has(this.isMajorElectiveCourses[degreeRqmnts[0]][i])){
        ...
        if(!openElectiveCourseAdded){
            for(var i = 0; i < this.isOpenElectiveCourses.length && cpqCounter < numCPQ && oeCounter < 1; i++){
                if(/* DATABASE CALL: courseAvailThisTerm(this.isOpenElectiveCourses[i], quarter.term) && */ !courseSet.has(this.isOpenElectiveCourses[i])){
        ...
        if(!capstoneAdded)
            if(/* DATABASE CALL: courseAvailThisTerm("IS 577", quarter.term) && */ cpqCounter < numCPQ && !courseSet.has("IS 577")){
  ...
  getDegreeRqmnts(degree : string){
	switch(degree){
		...
		case "Information Systems: Business Analysis/Systems Analysis": {
			return [ this.ISConcentration.BASA, this.ISNumMajorElective.BASA ]; //this.Foundation.IS, 
		}
  ...
  buildQuarterList(beginQuarter : string, numQuarters : number){
	...
	for(i = 0; i < numQuarters; i++){
		...
		quarterList[i] = new Quarter(currentTerm);
	}
