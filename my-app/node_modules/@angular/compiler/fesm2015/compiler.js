 ...
 *
 */
const VERSION = new Version('6.0.0-rc.4');
...
function compileComponent(outputCtx, component, pipes, template, reflector, bindingParser, mode) {
    ...
    function addDirectiveDependency(ast) {
        const importExpr$$1 = outputCtx.importExpr(ast.directive.type.reference);
        const uniqueKey = importExpr$$1.value.moduleName + ':' + importExpr$$1.value.name;
    ...
    const templateFunctionExpression = new TemplateDefinitionBuilder(outputCtx, outputCtx.constantPool, reflector, CONTEXT_NAME, BindingScope.ROOT_SCOPE, 0, component.template.ngContentSelectors, templateTypeName, templateName, pipeMap, component.viewQueries, addDirectiveDependency, addPipeDependency)
...
function addDependencyToComponent(outputCtx, summary, set, exps) {
    const importExpr$$1 = outputCtx.importExpr(summary.type.reference);
    const uniqueKey = importExpr$$1.value.moduleName + ':' + importExpr$$1.value.name;
...
class TemplateDefinitionBuilder {
    ...
    visitEmbeddedTemplate(ast) {
        ...
        // e.g. C(1, C1Template)
        this.instruction(this._creationMode, ast.sourceSpan, Identifiers$1.containerCreate, literal(templateIndex), ...trimTrailingNulls(parameters));
        // Generate directives
        this._visitDirectives(ast.directives, variable(CONTEXT_NAME), templateIndex);
        // Create the template function
        const templateVisitor = new TemplateDefinitionBuilder(this.outputCtx, this.constantPool, this.reflector, templateContext, this.bindingScope, this.level + 1, this.ngContentSelectors, contextName, templateName, this.pipes, [], this.addDirectiveDependency, this.addPipeDependency);
